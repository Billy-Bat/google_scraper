...
def find_element_by_inner_text(wd: webdriver.Firefox, inner_text: str):
    return wd.find_elements(By.XPATH, f"//*[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), '{inner_text}')]")        
...
def scrape_html_exhibitions_offi(content: str) -> List[ExhibitionData]:
    ...
    exhibitions: List[Tag] = soup.find_all('div', class_='mini-fiche-details d-flex has-padding-20')
    full_res = []
    for exhibition in exhibitions:
        exhibition_name: str = exhibition.find('span', itemprop='name').text.strip()
        exhibition_place: str = exhibition.find('div', class_='event-place pt-1').find('a').text.strip()
        exhibition_start_date: str = exhibition.find('meta', itemprop='startDate')['content']
        exhibition_end_date: str = exhibition.find('meta', itemprop='endDate')['content']
        full_res.append({
            "name": exhibition_name,
            "place": exhibition_place,
            "exhibition_start_date": datetime.strptime(exhibition_start_date, '%Y-%m-%d'),
            "exhibition_end_date": datetime.strptime(exhibition_end_date, '%Y-%m-%d')
        })
    return full_res
    
def crawl_exhibitions_offi_next_page(wb: webdriver.Firefox, is_first_page: bool = False) -> int:
    CURRENT_PAGE_CLASS = 'page-item active'
    current_page_number_ref_tag = wb.find_element(By.XPATH, f"//li[@class='{CURRENT_PAGE_CLASS}']/a")
...
with GoogleScraper(lang = "fr", extra_options = OPTIONS) as scraper:
    ...
    current_page = 1
    while True:
        current_page = crawl_exhibitions_offi_next_page(wb=wd, is_first_page=(current_page == 1))
        if current_page == 0:
            break
        res.extend(scrape_html_exhibitions_offi(content=content))
    import pprint
